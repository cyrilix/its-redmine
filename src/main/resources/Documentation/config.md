...
Example:
    ...
    match = "#([1-9][0-9]*)"
    html = "<a href=\"http://localhost:3000/issues/$1\">#$1</a>"
...
have match group 1 spanning over the whole “(refs #4711)”.
...
In order for Gerrit to connect to Redmine Rest api, host and apiKey
are required in your gerrit.config / secure.config under the [its-redmine] section.

Example:

    [its-redmine]
    host=http://myredmine.org
    apiKey=123456789
    

Redmine credentials and connectivity details are asked and verified during the Gerrit init.
...
Gerrit init example:
    ...
    ***
    ...
    Redmine issue number regex      #([1-9][0-9]*):
...
+ 'association':
    ...
    **Fixes-Issue: issue 4711**
    then a property 'association' with value _footer-Fixes-Issue_ would
...
+ 'added@<Association-Value>':::
      ...
      RevUpdatedEvents)
      _issue id occurs at '<Association-Value>' in the most recent patch
